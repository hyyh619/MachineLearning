...
def GetTFRecords(tfrecords):
    ...
    files = fd.FindTFRecordsFiles("./Samples", "train")
...
def Training(train_tfrecords, test_tfrecords):
    ...
    #test_features = GetTFRecords(test_tfrecords)

#    files = tf.train.match_filenames_once(train_tfrecords)
#    filename_queue = tf.train.string_input_producer(files, shuffle=True)
#    reader = tf.TFRecordReader()
#    _, serialized_example = reader.read(filename_queue)
...
#                    'label':tf.FixedLenFeature([], tf.int64)})
    ...
    # heights  = train_features['height']
#    heights = features['height']
#    widths  = features['width']
#    depths  = features['depth']]
    ...
    y = net.inference(example_batch, True, regularizer)
